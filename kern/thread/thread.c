...
thread_create(const char *name)
{
	...
	thread->interested_pid = NULL;
	
	thread->child_pid = NULL;
	
	thread->parent = NULL;
	
	thread->sem = sem_create("asdf", 1);
	...
	while(threads[counter].pid != NULL) { //threads[counter].pid != NULL alternative
		counter++;
		if(counter == 511) {
			counter = 0;
		}
...
struct thread findthread(int pid){
	int i;
	int found;
	int pidcheck;
	for(i=0;i<512;i++) {
		if(threads[i].pid != NULL){ //Need to initialize threads[]
			pidcheck = (int)(threads[i].pid);
			if(pidcheck == pid){
				found = i;
				break;
			}
...
/*
 * Thread initialization. Initialize threads[]
 */
struct thread *
